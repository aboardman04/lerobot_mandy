...
def main():
    ...
    # ------------------------------------------------------------------
    def hook_vision(module, inputs, output):
    ...
    # ------------------------------------------------------------------
    # 2. Capture Resampled Image Tokens from the SmolVLM Connector
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 3. Capture Full Prefix (final token sequence fed to the model)
    # ------------------------------------------------------------------
    ...
    batch = {
        ...
        "action": item["action"],
        "task": "pick and place object",  # string so tokenization works
    }
    ...
    for i, t in enumerate(captured["vision_encoder_tokens"]):
        print(f"   Camera {i}: {t.shape[1]} tokens (Shape: {tuple(t.shape)})")

    # ------------------------------------------------------------------
    # Resampled Image Tokens
    # ------------------------------------------------------------------
    ...
    print(f"   Total Tokens: {prefix.shape[1]} (Shape: {tuple(prefix.shape)})")
    ...
    # <image> and </image> per camera
    special_tokens = 2 * len(captured["resampled_image_tokens"])

    # State projection is typically a single token
    state_tokens = 1
